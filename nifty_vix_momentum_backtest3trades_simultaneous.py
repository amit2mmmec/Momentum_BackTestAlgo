...
def backtest_momentum(nifty_min1: pd.DataFrame, vix_min1: pd.DataFrame, tf_min: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for day, day_df in tf.groupby('date_only'):
        ...
        for i in range(2, len(day_df)):
            ...
            # Check entry if flat
            # Entry on signal + breakout of prior bar high/low to avoid chop
            if row['long_signal']:
                trig = row['high'] > day_df.loc[i-1,'high']
                if trig:
                    ...
                    trades.append(dict(
                        ...
                        pnl_pts=pnl,
                        vix_at_entry=float(vix_val) if pd.notna(vix_val) else np.nan,
                        vix_bucket=vix_bucket(float(vix_val)) if pd.notna(vix_val) else 'NA'
                    ))
                    trades_taken += 1

            elif row['short_signal']:
                trig = row['low'] < day_df.loc[i-1,'low']
                if trig:
...
if __name__ == "__main__":
    # Simple CLI
    import argparse
    p = argparse.ArgumentParser(description="Momentum Intraday Backtest for NIFTY + VIX")
