...
def generate_monthly_pnl_table(trades_df: pd.DataFrame, tf_min: int, writer, startrow: int):
    """
    Generate monthly P&L heatmap table for a given timeframe.
    Writes directly to Excel using the provided writer (xlsxwriter engine).
    """
    if trades_df.empty:
        return startrow
    ...
    monthly = trades_df.groupby(['Year','Month'])['pnl_pts'].sum().reset_index()
    pivot = monthly.pivot(index='Year', columns='Month', values='pnl_pts').fillna(0)
    ...
    pivot.columns = month_names + ['Total']
    
    all_years = list(range(2015,2026))
    pivot = pivot.reindex(all_years, fill_value=0)
    pivot.index.name = 'Year'
    
    header_df = pd.DataFrame({pivot.columns[0]: [f"{tf_min}-min Timeframe"]})
    header_df.to_excel(writer, sheet_name="Sheet1", startrow=startrow, index=False, header=False)
    
    pivot_reset = pivot.reset_index()
    pivot_reset.to_excel(writer, sheet_name="Sheet1", startrow=startrow+1, index=False)
    
    # === Apply conditional formatting ===
    worksheet = writer.sheets["Sheet1"]
    nrows, ncols = pivot_reset.shape
    
    # Data range (exclude "Year" col)
    data_start_row = startrow + 2
    data_end_row = startrow + 1 + nrows
    data_start_col = 1
    data_end_col = ncols - 1  # exclude Year
    
    data_range = xl_range(data_start_row, data_start_col, data_end_row, data_end_col)
    
    worksheet.conditional_format(data_range, {
        'type': '3_color_scale',
        'min_color': "#FF9999",  # red for worst
        'mid_color': "#FFFFFF",  # white for 0
        'max_color': "#99FF99"   # green for best
    })
    
    return startrow + nrows + 3
...
def backtest_momentum(nifty_min1: pd.DataFrame, vix_min1: pd.DataFrame, tf_min: int) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # ✅ Use pre-computed indicators directly from CSV
    tf['long_signal']  = (tf[f'EMA20_{tf_min}min'] > tf[f'EMA50_{tf_min}min']) & \
        (tf[f'EMA20_{tf_min}min_slope'] > 0) #& (tf[f'EMA20_{tf_min}min_ROC5'] > 0)

    tf['short_signal'] = (tf[f'EMA20_{tf_min}min'] < tf[f'EMA50_{tf_min}min']) & \
        (tf[f'EMA20_{tf_min}min_slope'] < 0) #& (tf[f'EMA20_{tf_min}min_ROC5'] < 0)

    tf['atr'] = tf[f'ATR14_{tf_min}min']
        ...
        while i < len(day_df) and trades_taken < MAX_TRADES_PER_DAY:
            ...
            if day_df.loc[i-1,'long_signal']:
                ...
                if not hit.empty:
                    ...
                    entry_idx = day_df.index[day_df['date'] >= entry_time][0]
                    j = entry_idx
...
def run(nifty_csv: str, vix_csv: str, out_prefix: str = "momo"):
    ...
    timeframes = [5, 15, 60]
    all_summaries = []
    all_trades = {}  # keep trades per TF for heatmap
    ...
    for tf in timeframes:
        ...
        all_trades[tf] = trades_df  # keep for heatmap
    ...
    # === NEW: Write summary + monthly P&L tables into one Excel ===
    excel_path = f"{out_prefix}_results.xlsx"
    with pd.ExcelWriter(excel_path, engine="xlsxwriter") as writer:
        # write summary first
        summary_df.to_excel(writer, sheet_name="Sheet1", startrow=0, index=False)

        # monthly P&L tables stacked below
        row_ptr = len(summary_df) + 3
        for tf in [5, 15, 60]:
            row_ptr = generate_monthly_pnl_table(all_trades[tf], tf, writer, row_ptr)
